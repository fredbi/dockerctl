...
// makeOperationSessionSessionCmd returns a command to handle operation session
...
func parseOperationSessionSessionResult(respErr error) (string, error) {
	if respErr != nil {
		...
		eresp1, ok := iResp1.(*session.SessionBadRequest)
		if ok {
		...
		eresp2, ok := iResp2.(*session.SessionInternalServerError)
		if ok {
			// the error response has a payload
