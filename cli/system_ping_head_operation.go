...
// makeOperationSystemSystemPingHeadCmd returns a command to handle operation systemPingHead
...
func parseOperationSystemSystemPingHeadResult(resp0 *system.SystemPingHeadOK, respErr error) (string, error) {
	if respErr != nil {
		...
		eresp0, ok := iResp0.(*system.SystemPingHeadOK)
		if ok {
		...
		eresp1, ok := iResp1.(*system.SystemPingHeadInternalServerError)
		if ok {
			// the error response has a payload
