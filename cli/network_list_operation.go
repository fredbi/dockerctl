...
// makeOperationNetworkNetworkListCmd returns a command to handle operation networkList
...
func registerOperationNetworkNetworkListFiltersParamFlags(cmdPrefix string, cmd *cobra.Command) error {
	...
	flagFiltersDescription := `JSON encoded value of the filters (a ` + "`" + `map[string][]string` + "`" + `) to process on the networks list. Available filters:
...
func parseOperationNetworkNetworkListResult(resp0 *network.NetworkListOK, respErr error) (string, error) {
	if respErr != nil {
		...
		eresp0, ok := iResp0.(*network.NetworkListOK)
		if ok {
		...
		eresp1, ok := iResp1.(*network.NetworkListInternalServerError)
		if ok {
			// the error response has a payload
