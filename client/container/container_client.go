...
// New creates a new container API client with basic auth credentials.
...
// New creates a new container API client with a bearer token for authentication.
// It takes the following parameters:
// - host: http host (github.com).
// - basePath: any base path for the API client ("/v1", "/v3").
// - scheme: http scheme ("http", "https").
...
// WithAcceptApplicationxTar sets the Accept header to "application/x-tar".
func WithAcceptApplicationxTar(r *runtime.ClientOperation) {
	r.ProducesMediaTypes = []string{"application/x-tar"}
}
...
func (a *Client) PutContainerArchive(params *PutContainerArchiveParams, opts ...ClientOption) (*PutContainerArchiveOK, error) {
	...
	op := &runtime.ClientOperation{
		...
		PathPattern:        "/containers/{id}/archive",
		ProducesMediaTypes: []string{"application/json", "text/plain"},
		ConsumesMediaTypes: []string{"application/x-tar", "application/octet-stream"},
		Schemes:            []string{"http", "https"},
		Params:             params,
